...
5. The single-inference gate verifies that no other prediction is running. The selected slot then takes a non-blocking snapshot of the newest eligible frame already present in its stream and releases the buffer lock. If that stream has no frame newer than its last inferred sequence, the slot is missed and the scheduler advances immediately; it never waits for a frame or drains a queue.
